...
(()=>{
  ...
  const sceneInfo = [
    ...
    },
    {
      ...
      values: {
        ...
        messageC_translateY_out: [0, -20, { start: 0.94, end: 0.98 }],
        messageA_opacity_out: [1, 0, { start: 0.4, end: 0.45 }],
        messageB_opacity_out: [1, 0, { start: 0.8, end: 0.85 }],
        messageC_opacity_out: [1, 0, { start: 0.94, end: 0.98 }],
  ...
  function loadImageOfScene2() {
    ...
    for (let i = 0; i < sceneInfo[2].values.videoImageCount; i++){
      let imgElem = new Image();
      imgElem.src = `./video/004/cider_${1293 + i}.jpg`;
      imgElem.addEventListener('load', () => {
  ...
  function checkMenu() {
    if(yOffset > 44){
      ...
      if(yOffset > sceneInfo[0].scrollHeight){
        document.body.classList.add('local-nav-sticky-dark');  
      }else{
        document.body.classList.remove('local-nav-sticky-dark');  
      }
    }else{
